...
class ZurekComparator:
    ...
    def _reduce_to_primitive(self, atoms):
        cell, scaled_pos, numbers = spglib.standardize_cell((atoms.cell, atoms.get_scaled_positions(), atoms.numbers), symprec=self.symprec, to_primitive=True)
...
class ASEComparator:
    ...
    def looks_like(self, ind1, ind2):
        ...
        if self.to_primitive:
            lattice, scaled_positions, numbers = spglib.find_primitive((ind1_.cell, ind1_.get_scaled_positions(), ind1_.numbers), symprec=self.symprec)
            ind1_ = Atoms(cell=lattice, scaled_positions=scaled_positions, numbers=numbers, pbc=ind1.pbc)
            ind2_ = []
            for ind in ind2:
                lattice, scaled_positions, numbers = spglib.find_primitive((ind.cell, ind.get_scaled_positions(), ind.numbers), symprec=self.symprec)
