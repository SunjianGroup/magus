...
from .writeresults import write_yaml, read_yaml
...
def calc_vasp_parallel(calcNum, calcPop, parameters, prefix='calcVasp'):
    ...
    runVasp.write("from csp.writeresults import write_yaml, read_yaml\n")
    ...
    runVasp.write("initPop = read_yaml('initPop.yaml')\n")
    runVasp.write("optPop = calc_vasp(calcs, initPop, )\n")
    runVasp.write("write_yaml('optPop.yaml', optPop)")
    runVasp.close()
    ...
    for i in range(numParallel):
        ...
        write_yaml('initPop.yaml', tmpPop)
...
def calc_gulp_parallel(calcNum, calcPop, parameters,):
    ...
    runGulp.write("from csp.writeresults import write_yaml, read_yaml\n")
    runGulp.write("initPop = read_yaml('initPop.yaml')\n")
    runGulp.write("optPop = calc_gulp({}, initPop, {}, {!r}, {!r})\n".format(calcNum, pressure, exeCmd, inputDir))
    runGulp.write("write_yaml('optPop.yaml', optPop)")
    runGulp.close()
...
def jobs_stat(runJobs):
    ...
    for jobID in runJobs:
        try:
            stat = os.popen("bjobs %s | grep %s | awk '{print $3}'"% (jobID, jobID)).readlines()[0][:-1]
        except:
            stat = 'DONE'
        # logging.info(jobID, stat)
        if stat == 'DONE':
            jobStat.append('DONE')
