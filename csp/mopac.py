...
from ase.calculators.calculator import FileIOCalculator, ReadError, Parameters, all_changes
...
class MOPAC(FileIOCalculator):
    ...
    def read_atoms_from_file(self, lines):
        ...
        if 1 in self.atoms.pbc:
            k = i - 2
            vecs = []
            for n in range(k-2, k+1):
                l = lines1[n].split()
                if l[1] == 'Tv':
                    vecs.append([float(l[2]), float(l[4]), float(l[6])])
    ...
    def read_results(self):
        ...
        for i, line in enumerate(lines):
            if line.find('TOTAL ENERGY') != -1:
                ...
                # with open('derivatives', 'w') as df:
                #     df.writelines(lines[i + 3:i + 3 + 3 * len(self.atoms)])
                if 'MOZYME' in self.parameters.task:
                    forces = [
                        [-float(line.split()[2]),
                        -float(line.split()[3]),
                        -float(line.split()[4])]
                        for line in lines[i + 5:i + 5 + len(self.atoms)]
                        ]
                    forces = np.array(forces) * kcal / mol
                else:
