...
# class KH:
#     def __init__(self, curPop, khParm=None):
#         self.curPop = curPop
#         self.khPop = list()
#         if not khParm:
#             khParm = parameters['khParm']
#         self.Vf = khParm['Vf']
#         self.Dmax = khParm['Dmax']
#         self.Nmax = khParm['Nmax']
#         self.Ct = khParm['Ct']
#         self.Wn = khParm['Wn']
#         self.Wf = khParm['Wf']
...
#         khPop = list()
#         # curPop = self.curPop[:]
#         curPop = [ind.copy() for ind in self.curPop]

#         oriLen = len(curPop) # origin length of curPop
#         curPop = calc_dominators(curPop)
#         curPop = sorted(curPop, key=lambda x:x.info['dominators'])
#         #curPop = curPop[:int(round((1 - randFrac)*oriLen))]
#         curLen = len(curPop)
...
#         bestInd = curPop[0]
#         worstInd = curPop[-1]
#         # fitMin = bestInd.info['MOGArank']
#         # fitMax = worstInd.info['MOGArank']
#         # fitMin = bestInd.info['fitness1']
#         # fitMax = worstInd.info['fitness1']
#         bestPos = bestInd.get_scaled_positions()
#         bestCell = bestInd.get_cell()

#         for ind in curPop:
#             ind.info['scaledFit'] = ind.info['fitness1'] - bestInd.info['fitness1'] + 1

#         fitMin = bestInd.info['scaledFit']
#         fitMax = worstInd.info['scaledFit']


#         # Food position
#         foodLen = int(0.5*len(curPop))
#         foodPop = curPop[:foodLen]
#         hNum = sum([1/ind.info['scaledFit'] for ind in foodPop]) # harmonic number
#         # hNum = sum([1/ind.info['fitness1'] for ind in foodPop]) # harmonic number
#         foodFit = foodLen/hNum
#         foodPos = sum([ind.get_scaled_positions()/ind.info['scaledFit'] for ind in foodPop])/hNum
#         foodCell = sum([ind.get_cell()/ind.info['scaledFit'] for ind in foodPop])/hNum

#         for i, ind in enumerate(curPop):
#             iPos = ind.get_scaled_positions()
#             iCell = ind.get_cell()
#             # fit = ind.info['MOGArank']
#             # fit = ind.info['fitness1']
#             fit = ind.info['scaledFit']

#             if 'Ni' not in ind.info:
#                 ind.info['Ni'] = [np.random.rand(len(ind), 3), np.random.rand(3, 3)]
#             Nold = ind.info['Ni'] # N_old: the last motion induced, [positions, cell]

#             if 'Fi' not in ind.info:
#                 ind.info['Fi'] = [np.random.rand(len(ind), 3), np.random.rand(3, 3)]
#             Fold = ind.info['Fi'] # F_old: the last foraging motion, [positions, cell]

#             if 'bestSelf' not in ind.info or ind.info['fitness1'] < ind.info['bestSelf'][2]:
#                 ind.info['bestSelf'] = [iPos, iCell, ind.info['fitness1']]
#                 # ind.info['bestSelf'] = [iPos, iCell, ind.info['MOGArank']]

#             ibestPos = ind.info['bestSelf'][0]
#             ibestCell = ind.info['bestSelf'][1]
#             ibestFit = ind.info['bestSelf'][2] - bestInd.info['fitness1'] + 1
#             #best positions, cell and fitness for ith krill(previously visited)

#             # motion by other krill
#             Kbest = (fit - fitMin)/(fitMax - fitMin)
#             Cbest = 2*random.random()
#             Npos = Nmax*Cbest*Kbest*(bestPos - iPos)
#             Npos += Wn*Nold[0]
#             Ncell = Nmax*Cbest*Kbest*(bestCell - iCell)
#             Ncell += Wn*Nold[1]

#             # foraging motion
#             Kfood = (fit - foodFit)/(fitMax - fitMin)
#             Cfood = 2*random.random()
#             ibestK = (fit - ibestFit)/(fitMax - fitMin)
#             Fpos = Vf*Cfood*Kfood*(foodPos - iPos)
#             Fpos += Vf*ibestK*(ibestPos - iPos)
#             Fpos += Wf*Fold[0]
#             Fcell = Vf*Cfood*Kfood*(foodCell - iCell)
#             Fcell += Vf*ibestK*(ibestCell - iCell)
#             Fcell += Wf*Fold[1]

#             # diffusion
#             Dpos = Dmax*np.array([[random.gauss(0, 0.2), random.gauss(0, 0.2), random.gauss(0, 0.2),]
#             for n in range(len(ind))])
#             Dcell = Dmax*np.random.rand(3, 3)

#             # new individual
#             # deltaPos = Npos + Fpos + Dpos
#             # deltaCell = Ncell + Fcell + Dcell
#             newPos = iPos + Ct*3*len(ind)*(Npos + Fpos + Dpos)
#             newCell = iCell + Ct*3*len(ind)*(Ncell + Fcell + Dcell)
#             khInd = Atoms(numbers=ind.get_atomic_numbers(), cell=newCell)
#             khInd.set_scaled_positions(newPos)
#             if not check_dist_individual(khInd, 0.5):
#                 khInd = gauss_mut(khInd)
#             khInd.info = ind.info.copy()
#             khInd.info['parentE'] = ind.info['enthalpy']
#             khInd.info['Ni'] = [Npos, Ncell]
#             khInd.info['Fi'] = [Fpos, Fcell]
#             khPop.append(khInd)
#         self.khPop = khPop
...
# def diff_mutate(parInd, aInd, bInd, beta = 0.3, GEP = False, record_enthalpy=True):
#     """
#     GEP: only for fixed composition
#     """

#     calcType = parameters['calcType']
#     symbols = parameters['symbols']
#     chdInd = parInd.copy() # child Individual

#     parCell, aCell, bCell = [ ind.get_cell() for ind in (parInd, aInd, bInd)]
#     chdCell = parCell + beta*(aCell - bCell)
#     chdInd.set_cell(chdCell, scale_atoms=True)

#     if GEP:
#         if calcType is 'fix':
#             maxNfm = parameters['numFrml'][-1]
#             parGene, aGene, bGene = [ ind.info['gene'] for ind in (parInd, aInd, bInd)]
#             parNfm, aNfm, bNfm = [ ind.info['numOfFormula'] for ind in (parInd, aInd, bInd)]
#             chdGene = {}
#             for i, sybl in enumerate(symbols):
#                 chdGene[sybl] = parGene[sybl] + beta*(aGene[sybl] - bGene[sybl])

#             chdNfm = int(round(parNfm + beta*(aNfm - bNfm)))
#             if chdNfm <= 0:
#                 chdNfm = 1
#             if chdNfm > maxNfm:
#                 chdNfm = maxNfm
#             chdInd.info['gene'] = chdGene
#             chdInd = gene_to_pos(chdInd, [chdNfm*n for n in chdInd.info['formula']])
#             chdInd.info['numOfFormula'] = chdNfm
#             chdInd.info['symbols'] = symbols
#             chdInd.info['formula'] = parameters['formula']

#         elif calcType is 'var': # uncompleted
#             parGene, aGene, bGene = [ ind.info['gene'] for ind in (parInd, aInd, bInd)]
#             parFrml, aFrml, bFrml = [ ind.info['Frml'] for ind in (parInd, aInd, bInd)]
#             chdGene = {}
#             chdFrml = []
#             for i, sybl in enumerate(symbols):
#                 chdGene[sybl] = parGene[sybl] + beta*(aGene[sybl] - bGene[sybl])
#                 numAt = int(round(parFrml[i] + beta*(aFrml[i] - bFrml[i])))
#                 chdFrml.append(numAt)

#             chdInd.info['gene'] = chdGene
#             chdInd = gene_to_pos(chdInd, chdFrml)
#             chdInd.info['symbols'] = symbols
#             chdInd.info['formula'] = chdFrml

#             pass


#     else:
#         parPos, aPos, bPos = [ ind.get_scaled_positions() for ind in (parInd, aInd, bInd)]
#         chdPos = parPos + beta*(aPos - bPos)
#         chdInd.set_scaled_positions(chdPos)

#     if record_enthalpy:
#         parentE = sum([ats.info['enthalpy'] for ats in (parInd, aInd, bInd)])/3
#         chdInd.info['parentE'] = parentE

#     return chdInd

# def diff_evo(curPop, GEP = False, record_enthalpy=True):
...
#     for i, ind in enumerate(curPop):
#         # i = curPop.index(ind)
#         # best = random.randint(0, paretolen -1)
#         nums = range(0, curlen)
#         nums.remove(i)
#         j, k = random.sample(nums, 2)
#         j, k = sorted([j, k])
#         newInd = diff_mutate(curPop[i], curPop[j], curPop[k], GEP=GEP, record_enthalpy=record_enthalpy)
#         newInd = crossover(newInd, bestInd)
#         newPop.append(newInd)


#     # if curlen < popSize:
#     #     composition = curPop[0].get_chemical_symbols()
#     #     symbols = list(set(composition))
#     #     formula = [composition.count(sybl) for sybl in symbols]

#     #     newPop.extend(build_struct(popSize - curlen, symbols, formula))
#     #     logging.debug('create '+str(popSize - curlen)+' random structures')

#     return newPop

# def bbo_cutcell(curPop, grid,):
#     """
#     BBO algorithm;
#     cut the cell;
#     fixed or variable composition
...
#     symbols = parameters['symbols']
#     popSize = parameters['popSize']
#     migrateFrac = parameters['migrateFrac']
#     mutateFrac = parameters['mutateFrac']
#     randFrac = parameters['randFrac']
#     calcType = parameters['calcType']
...
#         if index < mutateFrac*oriLen:
#             if random.randint(0, 1):
#                 bboInd = gauss_mut(ind)
#                 logging.info('index:%s Mutation' %index)
#             else:
#                 bboInd = exchage_atom(ind)
#                 logging.info('index:%s Exchange' %index)
#             bboInd.info['parentE'] = ind.info['enthalpy']
#             bboPop.append(bboInd)
#             continue
...
#         for k in range(numSiv):
#             imAts = [imAtom for imAtom in imPop[k]
#                     if siv[k][0]/k1 <= imAtom.a < (siv[k][0] + 1)/k1
#                     if siv[k][1]/k2 <= imAtom.b < (siv[k][1] + 1)/k2
#                     if siv[k][2]/k3 <= imAtom.c < (siv[k][2] + 1)/k3
#                     ]
...
#         #exchage atoms in bboPos
#         random.shuffle(bboPos)
...
#         elif calcType is 'var':

#             # curSybl = re.sub(r'[0-9]', ' ', bboInd.get_chemical_formula()).split()
#             # curFrml = re.sub(r'[^0-9]', ' ', bboInd.get_chemical_formula()).split()
#             # curDict = dict(zip(curSybl, curFrml))
#             bboInd.info['symbols'] = symbols
#             bboInd.info['formula'] = [len(syblDict[sybl]) for sybl in symbols]
#             bboInd.info['numOfFormula'] = 1
...
# def bbo_varcomp(curPop, grid,):
#     """ BBO algorithm; variable composition """
...
#     curPop = calc_dominators(curPop)
#     curPop = sorted(curPop, key=lambda x:x.info['dominators'])
#     curPop = curPop[:int(round(migrateFrac*curLen))]
#     curLen = len(curPop)
...
#     fitMax = curPop[-1].info['MOGArank']
#     for ind in curPop:
#         ind.translate([random.random(), random.random(), random.random()])
#         u = ind.info['MOGArank']/(fitMax + 1)
#         l = 1 - u
#         ind.info['u'] = u
#         ind.info['l'] = l
...
#         if index < mutateFrac*len(curPop):
#             bboInd = gauss_mut(ind)
#             bboPop.append(bboInd)
#             bboInd.info['parentE'] = ind.info['enthalpy']
#             logging.info('index:%s Mutation' %index)
...
#         # sortSybl = re.sub(r'[0-9]', ' ', bboInd.get_chemical_formula()).split()
#         # sortFrml = re.sub(r'[^0-9]', ' ', bboInd.get_chemical_formula()).split()
#         bboInd.info['symbols'] = symbols
#         bboInd.info['formula'] = [len(syblDict[sybl]) for sybl in symbols]
#         bboInd.info['numOfFormula'] = 1
...
# def bbo_fixcomp(curPop, grid,):
#     """ BBO algorithm; fixed composition """
...
# def bbo_fixcomp_old(curPop, popSize, numFrml):

#     curLen = len(curPop)
#     curPop = calc_dominators(curPop)
#     curPop = sorted(curPop, key=lambda x:x.info['dominators'])
#     # copyPop = [ atoms.copy() for atoms in curPop]
#     bboPop = []
#     symbols = curPop[0].info['symbols']
#     formula = curPop[0].info['formula']
#     syblDict = dict(zip(symbols, formula))
#     # numAtoms = len(curPop[0])/curPop[0].info['numOfFormula']

#     for ind in curPop:
#         u = curPop.index(ind)/curLen
#         l = 1 - u
#         ind.info['u'] = u
#         ind.info['l'] = l

#     for ind in curPop:
#         numIn = curPop.index(ind)
#         imIndex = []
#         count = 0
#         while count < numIn:
#             i = random.randrange(curLen)
#             if i != numIn and random.random() < curPop[i].info['l']:
#                 imIndex.append(i)
#                 count += 1

#         for j in range(curLen - numIn):
#             imIndex.append(numIn)
#         logging.info(imIndex)

#         imSet = set(imIndex)
#         # imDict = dict(zip(imSet, [imIndex.count(index)/float(curLen)
#         #         for index in imSet]))

#         bboCell = np.zeros((3,3))
#         nfm = 0 # number of formula
#         for k in imIndex:
#             bboCell = bboCell + curPop[k].get_cell()/curLen
#             nfm = nfm + curPop[k].info['numOfFormula']/curLen
#         nfm = int(round(nfm))

#         imDict = dict(zip(imSet,
#                 [int(round(imIndex.count(index)/curLen*nfm)) # number of formula emigrate from the individual
#                 for index in imSet]))

#         nfm = 0
#         for (key, value) in imDict.items():
#             if value > curPop[key].info['numOfFormula']:
#                 logging.info("Warning: Call for %s formula, but structure %s is too small!" % (value, key))
#             nfm += value

#         if nfm == 0:
#             bboInd = ind.copy()
#             bboPop.append(bboInd)
#             logging.info("Fail in BBO. Keep the Individual %s." %numIn)
#             continue

#         strFrml = ''
#         for sybl in symbols:
#             strFrml = strFrml + sybl + str(syblDict[sybl]*nfm)
#         bboInd = Atoms(strFrml, cell = bboCell, pbc = True)

#         # atomDict = dict(zip(symbols, [syblDict[sybl]*nfm]))

#         imList = [ x for x in imSet if imDict[x] > 0]
#         imList = sorted(imList, key = lambda x:imDict[x], reverse = True)

#         for n in imList:
#             emAts = [] # atoms which emigrate from curPop[n]
#             for (key, value) in syblDict.items():
#                 emAts = random.sample(curPop[n], value*imDict[n])
#                 imAts = [at for at in bboInd if at.tag == 0 and at.symbol == key]
#                 for atom in emAts:
#                     imAtom = random.choice([at for at in imAts if at.tag == 0])
#                     imAtom.a = atom.a
#                     imAtom.b = atom.b
#                     imAtom.c = atom.c
#                     imAtom.tag = 1
...
#         inputSybl = curPop[0].info['symbols']
#         inputFrml = curPop[0].info['formula']

#         bboPop.extend(build_struct(popSize - curLen, inputSybl, inputFrml, random.sample(numFrml, 1)))
#         logging.info('create '+str(popSize - curLen)+' random structures')
...
# def bbo_old(curPop, paretoPop, popSize):

#     # allPop = curPop[:].extend(paretoPop)
#     newPop = curPop[:]
#     # allLen = len(allPop)
#     curLen = len(curPop)
#     paretoLen = len(paretoPop)
...
#         if dominators > maxDom:
#             maxDom = dominators

#             # logging.info("dominators=", dominators)

#     # logging.info("maxDom=", maxDom)

#     newPop = sorted(newPop, key=lambda x:x.info['dominators'])

#     for ind in newPop:
#         u = newPop.index(ind)/curLen
#         # u = ind.info['MOGArank']
#         l = 1 - u
#         ind.info['u'] = u
#         ind.info['l'] = l

#     copyPop = [ atoms.copy() for atoms in newPop]

#     for ind in newPop:
#     #    logging.info('***********************')
#     #    logging.info("dominators=", ind.info['dominators'])
#     #    logging.info('index:', newPop.index(ind))
#         u = ind.info['u']
#         l = ind.info['l']
#         numIn = int(u*len(ind))
#     #    logging.info('u=', u, 'l=', l)
#     #    logging.info('numIn=', numIn)
#         if numIn == 0:
#             # best = random.randint(0, paretolen -1)
#             nums = range(0, curLen)
#             nums.remove(newPop.index(ind))
#             a, b = random.sample(nums, 2)
#             ind = crossover(ind, random.choice(paretoPop[:]))
#             ind = diff_mutate(ind, copyPop[a], copyPop[b])
#             ind = ind.copy()
#             # logging.info("Difference Evolution")
#             continue
#         imPop = [] # immigration pop
#         count = 0
#         while count < numIn:
#             i = random.randrange(curLen)
#             if i != copyPop.index(ind) and random.random() < copyPop[i].info['l']:
#                 imPop.append(copyPop[i])
#                 count = count + 1

#         imAtoms = random.sample(ind, numIn) # atoms which will be replaced by immigration pop
#         for j in range(numIn):
#             k = imAtoms[j].index
#     #        logging.info("====================")
#     #        logging.info(imAtoms[j].a, imAtoms[j].b, imAtoms[j].c)
#             imAtoms[j].a = imPop[j][k].a
#             imAtoms[j].b = imPop[j][k].b
#             imAtoms[j].c = imPop[j][k].c
#     #        logging.info(imAtoms[j].a, imAtoms[j].b, imAtoms[j].c)

#         newCell = 0.5*ind.get_cell()
#         for otherInd in imPop:
#             newCell = newCell + 0.5*otherInd.get_cell()/numIn
#         ind.set_cell(newCell, scale_atoms=True)

#         ind = crossover(ind, random.choice(paretoPop))

#     ###delete the bad individuals
#     migrateFrac = 0.7
#     newPop = newPop[:int(migrateFrac*curLen)]
#     curLen = len(newPop)
...
#         symbols = curPop[0].info['symbols']
#         formula = curPop[0].info['formula']
#         nfm = [curPop[0].info['numOfFormula']]

#         newPop.extend(build_struct(popSize - curLen, symbols, formula, nfm))
#         logging.info('create '+str(popSize - curLen)+' random structures')

#     return newPop

def bbo_basic(curPop):
    ...
    for ind in curPop:
        ...
        if numIn == 0:
            ...
            # a, b = random.sample(nums, 2)
            # ind = crossover(ind, random.choice(paretoPop[:]))
            # ind = diff_mutate(ind, curPop[a], curPop[b])
            # ind = ind.copy()
            # continue
        imPop = [] # immigration pop
        count = 0
        while count < numIn:
            i = random.randrange(curLen)
            if i != curPop.index(ind) and random.random() < curPop[i].info['l']:
                imPop.append(curPop[i])
                count = count + 1

        bboInd = ind.copy()
        imAtoms = random.sample(bboInd, numIn) # atoms which will be replaced by immigration pop
        for j in range(numIn):
            k = imAtoms[j].index
            imAtoms[j].a = imPop[j][k].a
            imAtoms[j].b = imPop[j][k].b
        ...
        bboCell = 0.5*bboInd.get_cell()
        for otherInd in imPop:
            bboCell = bboCell + 0.5*otherInd.get_cell()/numIn
        bboInd.set_cell(bboCell, scale_atoms=True)
        bboPop.append(bboInd)
...
# def pos_to_gene(inputInd):
#     """
#     Gene Expression Programming.
#     For random individuals, creat a new gene in ind.info.
#     For other individuals, renew the gene.
#     """

#     calcType = parameters['calcType']
#     symbols = parameters['symbols']
#     curSybl = inputInd.info['symbols']
#     curFrml = inputInd.info['formula']
#     nfm = inputInd.info['numOfFormula']
#     curDict = dict(zip(curSybl, [nfm*i for i in curFrml]))

#     if calcType is 'fix':
#         maxNfm = max(parameters['numFrml'])
#         formula = parameters['formula']
#         maxDict = dict(zip(symbols, [maxNfm*i for i in formula]))
#     elif calcType is 'var':
#         maxAt = parameters['maxAt']
#         # maxDict = dict(zip(symbols, [maxAt for sybl in symbols]))

#     geneInd = inputInd.copy()

#     try:
#         gene = geneInd.info['gene']
#         for sybl in symbols:
# #            logging.debug(sybl, curDict[sybl], len([(at.a, at.b, at.c) for at in geneInd if at.symbol is sybl]))
#             gene[sybl][:curDict[sybl]] = np.array([(at.a, at.b, at.c) for at in geneInd if at.symbol is sybl])

#     except:
#         gene = {}
#         for sybl in symbols:
#             if calcType is 'fix':
#                 gene[sybl] = np.random.rand(maxDict[sybl], 3)
#                 # logging.debug(sybl, curDict[sybl])
#                 # logging.debug(gene[sybl])
#                 # logging.debug([(at.a, at.b, at.c) for at in geneInd if at.symbol is sybl])
#                 gene[sybl][:curDict[sybl]] = np.array([(at.a, at.b, at.c) for at in geneInd if at.symbol is sybl])
#             elif calcType is 'var':
#                 if sybl in curSybl:
#                     gene[sybl] = np.random.rand(maxAt, 3)
#                     gene[sybl][:curDict[sybl]] = np.array([(at.a, at.b, at.c) for at in geneInd if at.symbol is sybl])
#                 else:
#                     gene[sybl] = None

#     geneInd.info['gene'] = gene
#     return geneInd

# def gene_to_pos(inputInd, toFrml):
#     symbols = parameters['symbols']
#     gene = inputInd.info['gene']
#     cell = inputInd.get_cell()
#     logging.info(toFrml)

#     strFrml = ''
#     genePos = []
#     for n, sybl in enumerate(symbols):
#         if toFrml[n] > 0:
#             strFrml = strFrml + "%s%s" %(sybl, toFrml[n])
#             genePos += list(gene[sybl][:toFrml[n]])
#             # logging.debug('len of genePos:', len(genePos))

#     genePos = np.array(genePos)
#     logging.debug('posShape', genePos.shape)
#     logging.debug(strFrml)
#     geneInd = Atoms(strFrml, cell = cell, pbc = True,)
#     geneInd.set_scaled_positions(genePos)
#     geneInd.info['gene'] = gene
